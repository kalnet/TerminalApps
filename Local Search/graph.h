...
// Copy constructor
{
   setNode(n.getId(), n.getWeight(), n.isMarked(), n.isVisited(), n.getColor());
}
...
{
   int n, m, i, a, b;
   fin >> n;  // read number of nodes
   fin >> m;  // read number of edges
...
void graph::setColor(int i, int c)
// Set the color of node i to c.
{
   if (i < 0 || i >= numNodes())
      throw rangeError("Bad value in graph::setColor");

   nodes[i].setColor(c);
}

int graph::getColor(int i) const
// Return the color of node i.
{
   if (i < 0 || i >= numNodes())
      throw rangeError("Bad value in graph::getColor");

   return nodes[i].getColor();
}

void graph::printSolution()
// Prints a graph coloring solution
{
   int numConflicts = 0;
   ...
   for (int i = 0; i < numNodes(); i++)
   {
      for (int j = 0; j < numNodes(); j++)
      {
	 if (isEdge(i,j) && getColor(i) == getColor(j))
	    numConflicts++;
      }
   }
...
// Return the degree of node i.  Assumes that each undirected edge is
// represented by a single directed edge.
{
   int d = 0;
   
   for (int i = 0; i < numNodes(); i++)
      if (isEdge(i,n) || isEdge(n,i))
	 d++;

   return d;
}

void graph::outFile(graph &G, int numColors, string whichOne)
{
	ofstream file;
	char buffer [5];
	char buffer2 [5];
   ...
   snprintf(buffer, 10, "%d", numNodes());
   snprintf(buffer2, 10, "%d", numColors);
	//itoa (numNodes(),buffer,10);
	//itoa (numColors,buffer2,10);
	ofileName = "color";
	ofileName += buffer ;
	ofileName += "-" ;
	ofileName += buffer2 ;
